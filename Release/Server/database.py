...
class Database:
    ...
    def addFileEntry(self, filename, remoteDir, checksum, user):
        ...
        with sq.connect(db_name) as conn:
            ...
            data = (filename, user, date, "1", "{'relationCount':0, 'relations':[]}", user, date, remoteDir, checksum,)

            conn.execute(query, data)
            conn.commit()

    def updateFileEntry(self, filename, remoteDir, checksum, user, version):
        ...
        with sq.connect(db_name) as conn:
            ...
            query = '''UPDATE files SET version = ?, last_user = ?, last_modified = ?, checksum = ? WHERE filename = ?'''
            qdata = (version, user, modified, checksum, filename,)
    ...
    def createProject(self, json_data):
        ...
        # create folder for project or return false if exists
        if not os.path.exists(self.data_location + p_name):
            os.makedirs(self.data_location + p_name)
        else:
            return False
